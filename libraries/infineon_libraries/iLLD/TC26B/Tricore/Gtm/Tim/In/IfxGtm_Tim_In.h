...
typedef struct
{
    ...
    uint32          periodTick;                  /**< \brief 以时钟刻度为单位的周期值 */
    uint32          pulseLengthTick;             /**< \brief 以钟点为单位的占空比值 */
    boolean         dataCoherent;                /**< \brief TRUE，如果占空比和周期值是从同一时期测量的 */
    boolean         overflowCnt;                 /**< \brief TRUE if the last measurement show an overflow in CNT */
    boolean         newData;                     /**< \brief 当值被更新时为 "true"，如果CNT、CNTS计数器都没有溢出，则为 "true"。*/
    boolean         dataLost;                    /**< \brief 如果数据丢失，则为TRUE */
    ...
    IfxGtm_Tim_Ch   channelIndex;                /**< \brief 正在使用的TIM通道的索引。 */
