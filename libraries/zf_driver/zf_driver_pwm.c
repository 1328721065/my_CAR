...
//  @param      duty        PWM占空比
//  @return     void
//  Sample usage:           pwm_freq(ATOM0_CH7_P02_7,50, 5000);//设置频率为50HZ 占空比为百分之5000/GTM_ATOM0_PWM_DUTY_MAX*100
//                          GTM_ATOM0_PWM_DUTY_MAX宏定义在zf_gtm_pwm.h  默认为10000
//-------------------------------------------------------------------------------------------------------------------
void pwm_frequency(pwm_channel_enum pwmch, uint32 freq, uint32 duty)
{
    uint32 period;
    ...
    switch(atom_channel->atom)
    {
        case 0: duty = (uint32)((uint64)duty * period / 10000); break;
        case 1: duty = (uint32)((uint64)duty * period / 10000); break;
        case 2: duty = (uint32)((uint64)duty * period / 10000); break;
        case 3: duty = (uint32)((uint64)duty * period / 10000); break;
    }
